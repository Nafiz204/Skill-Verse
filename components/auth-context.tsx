...
export function AuthProvider({ children }: { children: React.ReactNode }) {
  ...
  const legacyAuthKey = "dig" + "igyan_user"

  useEffect(() => {
    // Check if user is already logged in (from localStorage or session)
    const storedUser = localStorage.getItem("skill_verse_user") || localStorage.getItem(legacyAuthKey)
    if (storedUser) {
      try {
        setUser(JSON.parse(storedUser))
        localStorage.setItem("skill_verse_user", storedUser)
  ...
  const signIn = async (email: string, password: string, userType: UserType) => {
    ...
    try {
      ...
      setUser(newUser)
      localStorage.setItem("skill_verse_user", JSON.stringify(newUser))
    } finally {
      setIsLoading(false)
    }
  }
  ...
  const signOut = async () => {
    ...
    try {
      ...
      setUser(null)
      localStorage.removeItem("skill_verse_user")
      localStorage.removeItem(legacyAuthKey)
    } finally {
      setIsLoading(false)
    }
