...
export class GoogleCalendarService {
  ...
  /**
  * Get user's primary calendar or create a dedicated Skill Verse calendar
   */
  async getOrCreateSkillVerseCalendar(): Promise<string> {
    const calendar = google.calendar({ version: 'v3', auth: this.oauth2Client })

    try {
      // First, check if "Skill Verse Learning Tasks" calendar already exists
      const calendarList = await calendar.calendarList.list()
      const skillVerseCalendar = calendarList.data.items?.find(
        (cal) => cal.summary === 'Skill Verse Learning Tasks'
      )

      if (skillVerseCalendar && skillVerseCalendar.id) {
        return skillVerseCalendar.id
      }

      // Create a new calendar for Skill Verse tasks
      const newCalendar = await calendar.calendars.insert({
        requestBody: {
          summary: 'Skill Verse Learning Tasks',
          description: 'Automatically synced learning tasks from Skill Verse',
          timeZone: 'UTC',
        },
      })
  ...
  async createTaskEvent(
    ...
    const event: CalendarEvent = {
      summary: `📚 ${taskTitle}`,
      description: taskDescription || 'Skill Verse Learning Task',
      start,
      end,
